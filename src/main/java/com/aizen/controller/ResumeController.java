...
public class ResumeController {
    ...
    private Resume readForm() {
        ...
        r.setTemplate(cbTemplate.getValue());
        ...
        for (String[] v : expSection.getValues()) {
            r.getExperiences().add(new Experience(at(v, 0), at(v, 1), at(v, 2), at(v, 3), at(v, 4)));
        }
        for (String[] v : eduSection.getValues()) {
            r.getEducations().add(new Education(at(v, 0), at(v, 1), at(v, 2), at(v, 3)));
        }
        for (String[] v : projSection.getValues()) {
            r.getProjects().add(new Project(at(v, 0), at(v, 1), at(v, 2)));
        }
        for (String[] v : certSection.getValues()) {
            r.getCertifications().add(new Certification(at(v, 0), at(v, 1), at(v, 2)));
        }
        for (String[] v : skillSection.getValues()) {
            r.getSkills().add(new Skill(at(v, 0)));
        }
        return r;
    }
    ...
    private void populate(Resume r) {
        ...
        try {
            ...
            cbTemplate.setValue("Modern".equalsIgnoreCase(r.getTemplate()) ? "Modern" : "Classic");
            ...
            for (Experience e : r.getExperiences()) {
                rows.add(new String[]{e.getCompany(), e.getRole(), e.getStartDate(), e.getEndDate(), e.getDescription()});
            }
            expSection.setValues(rows);

            rows = new ArrayList<>();
            for (Education e : r.getEducations()) {
                rows.add(new String[]{e.getInstitution(), e.getDegree(), e.getYear(), e.getGrade()});
            }
            ...
            for (Certification c : r.getCertifications()) {
                rows.add(new String[]{c.getName(), c.getIssuer(), c.getYear()});
            }
    ...
    private void onSave() {
        ...
        SaveTask<Resume> task = new SaveTask<>("Saving resume...", () -> resumeService.save(r, userId));
    ...
    private void onExportJson() {
        ...
        SaveTask<File> task = new SaveTask<>("Writing JSON...", () -> jsonService.exportResume(r, file));
        TaskManager.run(task, progress, f -> setStatus("JSON exported to " + f.getAbsolutePath(), false), this::fail);
    }
    ...
    private File chooseFile(String title, String initialName, boolean save, String desc, String pattern) {
        ...
        return save ? chooser.showSaveDialog(SceneManager.getStage()) : chooser.showOpenDialog(SceneManager.getStage());
    }
}